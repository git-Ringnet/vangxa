                        ...
                        @if($user->ownedPosts && $user->ownedPosts->count() > 0)
                            @foreach($user->ownedPosts as $ownedPost)
                                <li class="dropdown-item flex items-center space-x-2">
                                    <span class="{{ ['text-blue-500', 'text-green-500', 'text-orange-500', 'text-purple-500', 'text-red-500'][rand(0, 4)] }}">
                                        <i class="fa {{ $ownedPost->type == 'dining' ? 'fa-utensils' : 'fa-building' }}"></i>
                                    </span>
                    ...
                    <div class="user-info">
                        <div class="user-name">
                            ...
                            @if(!$isOwnProfile && Auth::check())
                                <button class="btn btn-sm follow-btn ml-2 {{ $isFollowing ? 'btn-secondary' : 'btn-primary' }}"
                        ...
                        <div class="user-stats">
                            ...
                            <a href="{{ route('user.followers', $user->id) }}" class="user-stat-link">
                                <span><span class="stat-count followers-count">{{ $followersCount }}</span> người theo dõi</span>
                            </a>
                                ...
                                @if ($user->trustlists !== null)
                                    ...
                                    <a href="{{ $post->post->getDetailUrlAttribute() }}" class="listing-card">
                                        <div class="listing-image-container">
                                            ...
                                            <form action="{{ route('trustlist.toggle', ['id' => $post->post->id]) }}" method="POST" class="trustlist-form">
                                                @csrf
                                                ...
                                                <div class="flex-grow">
                                                    ...
                                                    <div class="flex items-center justify-between">
                                                        <div class="flex text-warning">
                                                            @for ($i = 1; $i <= 5; $i++)
                                                                <i class="fas fa-star {{ $i <= $review->food_rating ? 'text-warning' : 'text-gray-300' }}"></i>
                                                            @endfor
                                                        </div>
                            ...
                            <div class="row">
                                <div class="col-md-8 mb-3">
                                    ...
                                    <div class="cropper-controls">
                                        <button type="button" class="btn btn-sm btn-outline-primary btn-control" data-method="rotate" data-option="-90" title="Xoay trái">
                                            <i class="fas fa-undo"></i>
                                        </button>
                                        <button type="button" class="btn btn-sm btn-outline-primary btn-control" data-method="rotate" data-option="90" title="Xoay phải">
                                            <i class="fas fa-redo"></i>
                                        </button>
                                        <button type="button" class="btn btn-sm btn-outline-primary btn-control" data-method="zoom" data-option="0.1" title="Phóng to">
                                            <i class="fas fa-search-plus"></i>
                                        </button>
                                        <button type="button" class="btn btn-sm btn-outline-primary btn-control" data-method="zoom" data-option="-0.1" title="Thu nhỏ">
                                            <i class="fas fa-search-minus"></i>
                                        </button>
                                        <button type="button" class="btn btn-sm btn-outline-secondary btn-control" data-method="reset" title="Đặt lại">
